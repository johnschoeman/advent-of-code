...
const isDrawValid = (input: string): boolean => {
  const [value, color] = input.split(" ")
  switch (color) {
    case "red":
      return Number(value) <= maxRed
    case "green":
      return Number(value) <= maxGrn
    case "blue":
      return Number(value) <= maxBlu
    default:
      return false
  }
}

const gameId = (line: string): number => {
  return pipe(
    line,
    String.match(/\d+/g),
...
// Returns id of game if game is valid and 0 if not
export const gameValue = (line: string): number => {
  ...
  return pipe(
    ...
    ReadonlyArray.every(isDrawValid),
    isValidGame => (isValidGame ? gameId(line) : 0),
  )
}
